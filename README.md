...
2.2.2. "interval" is an interval length of each grids. It is used to calculate a regression loss.

2.2.3. "sequence" is a length of a time-series data.

##### 2.3. training setting
2.3.1. "weight_decay" is a hyperparameter for L2 Regularization. The regularization is used to prevent an overfitting.

2.3.2. "lr" is a learning rate of your training process.

2.3.3. "batch_size" is a batch size of input data.

2.3.4. "shuffle" is used to shuffle dataset when loading a dataset. When it is set to True, the argument is conveyed to dataloader to shuffle the dataset.
...
2.4.3. "drop_rate" is a drop rate of your model layers while training.

2.4.4. "num_layers" is a number of Conv3d layers in 3DCNN model.

##### 2.5. vivit
2.5.1. "tubelet_size" is a size of each tubelet in vivit.

2.5.2. "hidden_size" is a dimensionality of the encoder layers and the pooler layer.

2.5.3. "num_hidden_layers" is a number of hidden layers in the Transformer encoder.

2.5.4. "num_attention_heads" is a number of attention heads for each attention layer in the Transformer encoder.

2.5.5. "intermediate_size" is a dimensionality of the "intermediate" (i.e., feed-forward) layer in the Transformer encoder.

2.5.6. "hidden_dropout_prob" is a dropout probability for all fully connected layers in the embeddings, encoder, and pooler.

2.5.7. "attention_probs_dropout_prob" is a dropout ratio for the attention probabilites.
...
#### 1. command : 
``` shell script
python tune.py --config ./cfg/tune.yaml 
```
...
2.1.4. "gpu_per_trials" is a number of GPU usage per trial. For example, if it is set to 0.5, 0.5 GPU is used for each trial, which means two trials will be utilized simultaneously with one GPU.
...
2.2.3. "sequence" is a length of your time-series data.
...
2.5.2. "num_samples" is a number of times to sample from the hyperparameter space.
...
- You can choose which metric to focus on, and use the "best hyperparameters" and weights in "best log dir" for the metric. 
...
#### 1. command : 
``` shell script
python diffusion.py --config ./cfg/diffusion.yaml 
```
...
#### 3. output : 
dataset_{visual_sample}.pkl, diffusion.yaml, result.log, sample figures(optional)